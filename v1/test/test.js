...
describe("BasicDutchAuction", function () {
    ...
    it('should not allow the highest bidder to refund their bid', async () => {
        ...
        await expect(auction.connect(bidder2).refundBid(bidder2.address)).to.be.revertedWith('Highest bidder cannot refund');
    });

    it('should not allow the bidder to refund their bid if auction has not ended', async () => {
        await expect(auction.connect(bidder2).refundBid(bidder2.address)).to.be.revertedWith('Auction has not ended yet');
    });
